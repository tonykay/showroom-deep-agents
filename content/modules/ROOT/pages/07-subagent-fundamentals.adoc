...
sequenceDiagram
    ...
    User->>Main: "Research REST vs GraphQL"
    Main->>Main: Reads subagent descriptions
    Main->>Task: task(subagent_type="researcher", description="...")
    Task->>Sub: Creates fresh agent with system_prompt
    Sub->>Sub: Works in isolated context
    Sub-->>Task: Returns result as string
    Task-->>Main: Result arrives as ToolMessage
    Main->>User: Integrates result into final response
....
