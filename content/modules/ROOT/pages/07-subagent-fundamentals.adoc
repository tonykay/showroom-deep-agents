...
You've learned state management and tool building. Now you'll master the third pillar of deep agents: https://docs.langchain.com/oss/python/deepagents/subagents[subagents]. This is where the framework truly shines—subagents solve the context bloat problem that causes long-running agents to degrade.
...
For a deeper dive into these failure modes, see https://www.dbreunig.com/2025/06/22/how-contexts-fail-and-how-to-fix-them.html[How Contexts Fail] and https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus[Context Engineering for AI Agents] from the Manus team.
...
TIP: This isn't just about avoiding errors — smaller subagents with focused context often *outperform* large agents with full history. Models reason better when given only the information relevant to the task at hand. Context isolation is a performance optimization, not just a memory management technique.

Here's the pattern:

image::subagent-delegation.svg[Subagent delegation pattern — Main Agent delegates tasks to Subagents with fresh context,align="center",link=self,window=blank,width=700]
...
The subagent is defined as a dictionary with three required fields (see the https://docs.langchain.com/oss/python/deepagents/subagents#configuration[full configuration reference]):
...
sequenceDiagram
    ...
    Note over Main: User asks a question
    Main->>Main: Reads subagent descriptions
    Main->>+Sub: task(subagent_type, description)
    Note over Sub: Fresh context — no history
    Sub->>Sub: Reasons and uses tools
    Sub-->>-Main: Result as ToolMessage
    Note over Main: Integrates result
....
...
- `subagent_type`: Which subagent to invoke (matches the subagent's `name` field)
- `description`: A natural-language description of what the subagent should do

The main agent decides _which_ subagent to call by reading the `description` fields you provided when defining them. It decides _what_ to ask by formulating the `description` argument based on the user's request.
