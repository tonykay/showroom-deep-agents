...
NOTE: The `execute` tool's subprocess has a minimal PATH. Commands like `python`, `uv`, or `node` may not be found even though they work in your terminal. The agent can usually figure this out (as it did above), but if you need specific tools, provide absolute paths in your prompt or system prompt.
