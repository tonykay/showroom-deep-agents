...
Notice the `virtual_mode=False` parameter — this tells the backend to work directly with the real filesystem. Without it, you'll get a deprecation warning. The `root_dir` scopes file operations to the `./workspace` directory.
...
backend = FilesystemBackend(root_dir="./workspace/myproject", virtual_mode=False)
