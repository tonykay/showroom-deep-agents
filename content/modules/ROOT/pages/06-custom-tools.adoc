...
The agent pauses before executing the `calculate` tool and waits for approval. Notice two requirements for interrupts:

* **`interrupt_on={"calculate": True}`** — tells the agent which tools need approval
* **`checkpointer=MemorySaver()`** — saves the agent's state when it pauses so it can resume after approval. Without a checkpointer, `get_state()` fails because there's nowhere to store the paused state.
