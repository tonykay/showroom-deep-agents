...
NOTE: Don't confuse `CLAUDE.md` with `AGENTS.md`. `CLAUDE.md` is read by Claude Code (your development tool). `AGENTS.md` is read by Deep Agents at runtime via `MemoryMiddleware` (as you learned in Module 8). They serve different audiences — one is for your dev assistant, the other is for the agents you build.
...
. Once it works, update your project's `CLAUDE.md` with the insight
