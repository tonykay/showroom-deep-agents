...
Let's examine the canonical example from the deepagents repository. Here's what a `subagents.yaml` file looks like:
...
First, define a simulated `web_search` tool:
...
Now create the loader function:
...
You'll see the main agent delegate to the researcher, which uses the `web_search` tool and returns synthesized findings.
...
== Exercise 5: Structuring Larger Projects
...
You'll see validation errors for the too-short description and system_prompt.

Now test with the valid config:
