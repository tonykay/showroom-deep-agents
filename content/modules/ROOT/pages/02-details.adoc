...
def delegation_trace(result):
    ...
    Prints each task() tool call: which subagent was called and what
    it was asked to do. Useful for understanding orchestration flow.
    """
    steps = []
    ...
    for i, (agent_name, desc) in enumerate(steps, 1):
        lines.append(f"  Step {i}: {agent_name}")
        lines.append(f"          {desc}...")
